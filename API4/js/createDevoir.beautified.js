...
var selecteur = function() {
    ...
    this.doObjectsCollide = function(a, b) {
        ...
        return !(aTop + aH < bTop || aTop > bTop + bH || aLeft + aW < bLeft || aLeft > bLeft + bW);
    };
...
var CreatePage = function() {
    ...
    this.copy = {};
    //objet jQuery contenant la copie des sélections lors des ctrl+C , ctrl+X
    ...
    this.init = function(txt) {
        ...
        */
        ...
        $("#addDrag").on("click", null, {
            element: "boite",
            type: "drag",
            codeHtml: '<div id="boiteindice" class="unselectable construction" type="drag" style="position:absolute;left:0px;top:0px;width:150px;height:100px;"><div id="dragindice" class="unselectable dvDrag" mode="cible" style="position:absolute;left:0px;top:0px;width:100%;height:100%;cursor:pointer;z-index:10"><span></span></div><script class="interactivite">$("#dragindice").draggable({containment:"#page", cursor: "pointer",   revert: false, zIndex: 1000,stop:addDragStop,start:addDragStart,scroll:false});</script></div>'
        }, $.proxy(this.addDiv, this));
    ...
    this.createPartie = function(event) {
        ...
        var html = '<div id="question' + numQuestion + '" class="question choices-border">' + '<div class="page questionid"></div></div>';
    ...
    this.setInteractivite = function(selecteur) {
        ...
        $(selecteur).filter("[type!=text_selectable][type!=scientifique]").on("dblclick", function(event) {
            event.stopPropagation();
            dv.boiteID = this.id;
            ...
            $("#textModif").dialog("option", "position", {
                my: "center center",
                at: "center center",
                of: document
            });
            $("#textModif").dialog("option", "buttons", [ {
                text: "Ok",
                click: function() {
                    var toto = $("#" + dv.boiteID + " span");
                    toto[0].innerHTML = CKEDITOR.instances.modifTextarea.document.$.body.innerHTML;
                    //$("#textModif textarea").val();
        ...
        if (typeZone != "drag") {
            $(selecteur).draggable(draggableOptions).resizable({
                //grid: (10, 10),
                alsoResize: ".selected",
                handles: "all",
                autoHide: false,
          ...
          case "scientifique":
            {
                ...
                $("#meEE").on("change", function(evt) {
                    if ($("#meEE").prop("checked")) {
                        $(maPage.selection.objets[0]).attr("statu", "modifiable").addClass("borderPlein").trigger("refresh");
                    }
                });
                $("#meEP").on("change", function(evt) {
                    if ($("#meEE").prop("checked")) {
                        $(maPage.selection.objets[0]).attr("statu", "non-modifiable").removeClass("borderPlein").trigger("refresh");
                    }
                });
                var html2 = corrige.setUI(event);
                break;
...
function setdropUIParameter(event) {
    $("#eMasque").on("change", function(evt) {
        if ($(this).prop("checked")) {
            $(maPage.selection.objets[0]).removeClass("zoneDepot").removeAttr("titre");
        } else {
            $(maPage.selection.objets[0]).addClass("zoneDepot").attr("titre", "");
        }
    });
